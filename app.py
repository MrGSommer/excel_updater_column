...
if non_num:
    ...
    # korrekte Masken für beide DataFrames
...
# nur gefilterte Zeilen für Updates
...
if "Fläche" in df_base.columns:
    st.sidebar.metric("Fläche O/U", f"{df_base['Fläche'].sum():.2f}", f"{df_upd_base['Fläche'].sum()-df_base['Fläche'].sum():+.2f}")
...
if st.button("Vergleich starten"):
    ...
    # nur gefilterte Indizes updaten
    idxs = df_out.index[mask_base]
    ...
    # Fallback + Toleranz
    if fb_cols:
        for i in idxs.difference(idx for idx,_ in [(x,x) for x in green|orange]):
            row = df_out.loc[i]
    ...
    for key in full_map.index.unique():
        if key not in {tuple(df_out.loc[i,c] for c in match_cols) for i in df_out.index}:
            u = full_map.loc[key]
