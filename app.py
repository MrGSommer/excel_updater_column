...
if uploaded_original and uploaded_update:
    try:
        ...
        group_col = st.selectbox("Gruppieren nach", options=common_cols,
        ...
        if "Fläche" in common_cols:
            ...
            st.dataframe(pd.merge(sum_o, sum_u, on=group_col, how="outer").fillna(0),
        ...
        fallback_columns = st.multiselect("🔁 Fallback-Matching-Spalten",
        ...
        existence_attrs = st.multiselect("🔍 Attribute für Existenzprüfung",
                                         options=[c for c in common_cols if c != "GUID"],
                                         help="Kombination muss im gefilterten Original vorhanden sein.")
        adopt_list = []
        if existence_attrs:
            # nur gefiltertes Original prüfen
            orig_set = {
                tuple(r[a] for a in existence_attrs)
                for _, r in df_original.iterrows()
            }
            # nur gefiltertes Update prüfen
            upd_sub = df_update.drop_duplicates(existence_attrs)
            missing_combos = upd_sub[~upd_sub.apply(
                lambda r: tuple(r[a] for a in existence_attrs) in orig_set, axis=1
            )][existence_attrs].drop_duplicates().reset_index(drop=True)

            if not missing_combos.empty:
                st.subheader("⚠️ Fehlende Kombinationen (im gefilterten Original)")
        ...
        if st.button("Vergleich starten"):
            ...
            for idx in range(len(df_orig)):
                ...
                if key in full_map.index:
                    ...
                    matched.add(key); idx_guid.add(idx)
                    for col in overwrite_columns:
                        ...
                        if pd.notna(nv) and nv != ov:
                            ...
                            green.add((idx+2, df_orig.columns.get_loc(col)+1))
                            updated=True
                    ...
                    if fb_key in fb_map.index:
                        ...
                        if fb is not None:
                            ...
                            if fullk in full_map.index and fullk not in matched:
                                matched.add(fullk); idx_fallback.add(idx)
                                updated_fb=False
                                for col in overwrite_columns:
                                    ...
                                    if pd.notna(nv) and nv != ov:
                                        df_orig.at[idx, f"{col} zuvor"] = ov
                                        df_orig.at[idx, col]             = nv
                                        blue.add((idx+2, df_orig.columns.get_loc(col)+1))
                                        updated_fb=True
                                for col in additional_columns:
            ...
            # 2) Toleranz-Matching
            orig_unm = {i for i in range(len(df_orig)) if i not in idx_guid|idx_fallback}
            ...
            for i,k,_ in pairs:
                if i not in idx_guid|idx_fallback and k not in matched:
                    u = full_map.loc[k]
                    u = u.iloc[0] if isinstance(u, pd.DataFrame) else u
                    for col in overwrite_columns:
                        ov = df_orig.at[i,col]
                        ...
                        orange.add((i+2, df_orig.columns.get_loc(col)+1))
                    for col in additional_columns:
                        df_orig.at[i, col] = u[col]
            ...
            for k in full_map.index.unique():
                if k not in matched:
                    u = full_map.loc[k]
                    u = u.iloc[0] if isinstance(u, pd.DataFrame) else u
                    # nur übernehmen, wenn in adopt_list (aus gefiltertem Update)
                    if existence_attrs:
            ...
            with pd.ExcelWriter(buf, engine="openpyxl") as writer:
                ...
                for (r,c) in blue:   ws.cell(row=r, column=c).fill = fmt["blue"]
